...
For UK federal minimum is £5.05 in 2005 and rises to £10 over 18 in 2025, which net is 9,696 a year. Benefits (universal credit starting in 2012) is GBP 2,847 per year (1517 single on universal). I take universal credit to error on the side of caution.Basic Jobseekers allowance in 2010 was 283.62 a month 3403.44
...
UK <- UK %>%
  filter(hh_incomey >= 3403) %>%
  filter(adjusted_income >= 3403) %>%
  rename(weight = hhdenui_xw)
...
ggplot(deu_props_long_clean, aes(x = year, y = percentage, color = technology)) +
  geom_line() +
  geom_point() +
  facet_wrap(~weighted_decile) +
  theme_minimal() +
...
```
Export for Overleaf--- return to fix 
...
### Different latex option ### 

options(modelsummary_format_numeric_latex = 'plain')

for (iso2 in c('GB', 'DE', 'US')) {
  sapply(1:5, function(q) {
    modelsummary(
      tech_penetration_fits[[iso2]][[q]] %>%
        (function(l) {
          names(l) <- str_to_title(names(l)) %>% str_replace('_', ' ')
          l
        }),
      stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01),
      # shape = 'rbind',
      ...
      # fmt = "%.3f",
      fmt = fmt_term(
        inflexion_year = function(x) as.integer(x),
        default = function(x) sprintf("%.2f", x)
      ),
      table_format = "latex",
      title = paste("Technology Adoption Parameters:", 
                    countrycode::countryname(iso2, 'iso2c', 'country.name')),
      gof_omit = 'AIC|BIC|Log|fin|Conv',
      coef_map = c(
        "country_max" = "Maximum Penetration by 2050",
        "growth_rate" = "Growth Rate",
        "inflexion_year" = "Inflection Year"
      ),
      note = c("* p < 0.1, ** p < 0.05, *** p < 0.01"),
      tex_wrap = c("begin{table}[H]", "\\centering", "\\small", "\\end{table}"),
      add_rows = NULL
    ) %>%
      print %>% capture.output() %>%
      str_split('\n') %>%
      tail(-5) %>% head(-2) %>%
      c(str_c("Quintile ", q, "& & & & & & \\\\ \\midrule"), .) %>%
      str_c(collapse = '\n')
  }) %>%
    str_c(collapse = '\\midrule \n') %>%
    str_c("\\begin{tabular}{l{2cm}p{1cm}p{1cm}p{1cm}p{1cm}p{1cm}p{1cm}} \\toprule
          ...
          \\end{tabular}") %>%
    readr::write_file(
      fs::path(output_dir, str_c("tech_penetration_fits_", iso2, ".tex"))
    )
}
...
DEU_energy$weighted_decile <-as.factor(DEU_energy$weighted_decile)
...
DEU_energy_by_decile_MWH %>%
        ggplot(aes(year, value, color = weighted_decile)) +
        geom_line() +
        ylim(0, NA) +
        facet_wrap(~variable) +
        theme_minimal() + 
...
```{r panel with fixed effects, echo = FALSE}
...
### Electricity #### 
model_uk_elec <- plm(elec_a_mwh ~ elec_price + gas_price + log(hh_incomey) + hhsize + 
...
model_us_elec <- plm(elec_a_mwh ~elec_price + gas_price + log(hh_income) + hh_size + 
                       hh_rooms + hh_own + us_state + weighted_decile + year, 
                     data = energy_data_us, model = "within", index = 
                       c("weighted_decile", "year"))

model_de_elec <- plm(elec_a_mwh ~elec_price + gas_price + log(hh_incomey) + hgtyp1hh + hgsize + hgowner + hgnuts1 + weighted_decile + year, 
...
create_country_vis <- function(country_code) {
        ...
        ggplot() +
                ...
                labs(
                        title = paste(country_code, "Historical and Projected Energy Consumption and Bills by Decile"),
                        subtitle = "Solid lines = historical data, Dashed lines = projections to 2050",
                        x = "Year",
                        y = NULL,
                        color = "Income Decile",
                        fill = "Income Decile"
...
create_enhanced_vis <- function(projections, historical_data, country_code) {
  ...
  country_name <- case_when(
    ...
    TRUE ~ country_code  # Fallback to code if not matched
  )
  ...
  ggplot() +
    ...
    # Customize appearance
    labs(
      title = paste(country_name, "baseline projection by income decile"),
...
```
#### Alternative for enhanced visualization to 2050 
```{r alternative enhanced visualization to 2050, include=FALSE}
create_enhanced_vis <- function(projections, historical_data, country_code) {
  # Get full country name
  country_name <- case_when(
    country_code == "DEU" ~ "Germany",
  ...
  historical <- historical_data %>%
    ...
    mutate(
      type = case_when(
        metric %in% c("baseline_gas", "baseline_elec", "total_consumption") ~ "Consumption (MWh)",
        TRUE ~ paste0("Annual Bills (", currency_label, ")")
      ),
      source = case_when(
        metric %in% c("baseline_gas", "annual_gas_bill") ~ "Gas",
        metric %in% c("baseline_elec", "annual_elec_bill") ~ "Electricity",
  ...
  projection <- projections %>%
    ...
    mutate(
      # CRITICAL FIX: Use the exact same label format as in historical data
      type = case_when(
        grepl("projected", metric) ~ "Consumption (MWh)",
        TRUE ~ paste0("Annual Bills (", currency_label, ")")
      ),
  ...
  ggplot() +
    ...
    # Customize appearance
    labs(
      title = paste(country_name, "historical and projected energy consumption and bills by income decile"),
      subtitle = "Solid lines = historical data, dashed lines = projections to 2050",
      x = "Year",
      y = NULL,
      color = "Income Decile",
    ...
    scale_color_viridis_d() +
    scale_fill_viridis_d() +
    # Use original y-axis settings (automatic limits)
    scale_y_continuous(limits = c(0, NA), expand = c(0, 0)) +
    theme(
      legend.position = "bottom",
      plot.title = element_text(face = "bold"),
...
```
### Distributional Analysis for Latex 
```{r distributional visualizaiton for latex, include = FALSE}
as.data.table(US)[, .(inc = weighted.mean(hh_income, weight)), by = 'weighted_decile'][order(weighted_decile)]
as.data.table(DEU)[, .(inc = weighted.mean(hh_incomey, weight)), by = 'weighted_decile'][order(weighted_decile)]
as.data.table(UK)[, .(inc = weighted.mean(hh_incomey, weight)), by = 'weighted_decile'][order(weighted_decile)]

convert_rds_to_latex <- function(rds_file, caption = NULL, digits = 2) {
  rds_file <- "Output/distributional_analysis.rds"
...
readRDS("Output/distributional_analysis.rds") %>% as.data.table %>%
  select(country, weighted_decile, 
         total_annual_bill_2025, total_annual_bill_2050,
         baseline_total_annual_bill_2025, baseline_total_annual_bill_2050,
...
compare_adoption_scenarios <- function(baseline, frontloaded, backloaded, country_code) {
  ...
  combined_data <- combined_data %>%
    mutate(income_group = case_when(
      weighted_decile == 1 ~ "Low Income (D1)",
      weighted_decile == 5 ~ "Middle Income (D5)",
      weighted_decile == 10 ~ "High Income (D10)",
  ...
  ggplot(combined_data, 
    ...
    labs(
      title = paste(country_name, "- Impact of Adoption Timing by Income Group"),
...
compare_relative_impacts <- function(baseline, frontloaded, backloaded, country_code) {
  ...
  ggplot(combined_data, 
    ...
    labs(
      title = paste(country_name, "- Relative Impact of Different Adoption Scenarios"),
