...
pacman::p_load(
  ...
  scales,         # Scale adjustments
  
  # Statistical Analysis
  survey,         # Survey data analysis
  nls2,           # Non-linear least squares
  minpack.lm,     # More robust non-linear fitting
  ...
  priceR,         # Price-related utilities
  quantmod,       # Financial time series
...
tech_penetration_fits[['DE']] %>% `names<-`(str_c("Quintile ", 1:5)) %>%
  modelsummary(stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01),
               shape = 'rbind',
               output = "gt")%>%
  gtsave("~/Documents/GitHub/EDITS/Output/tech_penetration_fits_DE.png")
...
### version for Overleaf 
output_dir <- "~/Documents/GitHub/EDITS/Output/"

# Generate PNG and .tex tables
create_model_summary(tech_penetration_fits[['DE']], "Germany", file.path(output_dir, "tech_penetration_fits_DE.tex"))

create_model_summary(tech_penetration_fits[['GB']], "United Kingdom", file.path(output_dir, "tech_penetration_fits_GB.tex"))

create_model_summary(tech_penetration_fits[['US']], "United States", file.path(output_dir, "tech_penetration_fits_US.tex"))
...
create_model_summary <- function(models, country, output_path) {
    models %>% 
      ...
      modelsummary(
        ...
        gof_omit = 'AIC|BIC|Log|fin|Conv',  # Remove these statistics
        coef_map = c(
...
energy_US <- US %>%
  ...
  summarise(
    avg_elec_exp = weighted.mean(elec_a_exp, weight, na.rm = TRUE),
    avg_gas_exp = weighted.mean(gas_a_exp, weight, na.rm = TRUE),
    n = n()
  ) %>%
  arrange(year, decile) %>%
  mutate(country = "US")
...
energy_UK <- UK %>%
  group_by(year, decile) %>%
  summarise(
    avg_elec_exp = weighted.mean(xpelecy, weight, na.rm = TRUE),
    avg_gas_exp = weighted.mean(xpgasy, weight, na.rm = TRUE),
    n = n()
  ) %>%
  arrange(year, decile)
energy_DEU <- DEU %>%
  group_by(year, weighted_decile) %>%
  summarise(
    average_elec_exp = weighted.mean(elec_a_exp, )
  )
...
energy_DEU <- DEU %>%
  ...
  filter(year != 2015) %>%
  summarise(
    avg_elec_exp = 12 * weighted.mean(hgelectr, weight, na.rm = TRUE),
    avg_gas_exp = 12 * weighted.mean(hgheat, weight, na.rm = TRUE),
    n = n()
  ) %>%
  arrange(year, decile)

energy_DEU <- DEU %>%
  mutate(hgheat = ifelse(hlf0084 < 0, NA, hgheat)) %>%
  group_by(year, decile) %>%
  filter(year != 2015) %>%
  summarise(
    avg_elec_exp = 12 * weightmean(hgelectr, na.rm = TRUE),
